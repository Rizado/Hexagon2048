...
from config import BG_COLOR, CX, CY, HEXAGON_R, BORDER_COLOR, TEXT_COLOR, BORDER_WIDTH, FONT_FACE, FONT_SIZE


def draw_hexagon(surface, x, y):
    pygame.draw.polygon(surface, BG_COLOR, [(x + HEXAGON_R * sin(2 * pi * i / 6), y + HEXAGON_R * cos(2 * pi * i / 6)) for i in range(6)],0)  # Вначале закрашиваем
...
def draw_cell(surface, x, y, text=0):
    ...
    draw_hexagon(surface, draw_x, draw_y)
